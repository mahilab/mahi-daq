    ...
    m_h(h)
{
    ...
    auto write_impl = [this](const ChanNum* chs, const int* counts, std::size_t n) {
        ...
        if (result == 0) {
            LOG(Verbose) << "Reset " << get_name() << " counts";
            return true;
        }
        else {
            LOG(Error) << "Failed to write " << get_name() << " counts " << get_quanser_error_message(result);
            return false;
        }
    };
    on_write.connect(write_impl);
    /// Write Quadratue Factors
    auto write_quad_impl = [this](const ChanNum* chs, const QuadFactor* quads, std::size_t n) {
        std::vector<t_encoder_quadrature_mode> converted_factors(n);
        for (int i = 0; i < n; ++i) {
            if (quads[i] == QuadFactor::X0)
                converted_factors[i] = ENCODER_QUADRATURE_NONE;
            else if (quads[i] == QuadFactor::X1)
                converted_factors[i] = ENCODER_QUADRATURE_1X;
            else if (quads[i] == QuadFactor::X2)
                converted_factors[i] = ENCODER_QUADRATURE_2X;
            else if (quads[i] == QuadFactor::X4)
                converted_factors[i] = ENCODER_QUADRATURE_4X;
            else {
                LOG(Error) << "QuadFactor X" << static_cast<int>(quads[i]) << " not supported by Quanser";
                return false;
            }
        }
        ...
        util::sleep(milliseconds(10));
        if (result == 0) {
            LOG(Verbose) << "Wrote " << get_name() << " quadrature factors";
            return true;
        }
        else {
            LOG(Error) << "Failed to write " << get_name() << " quadrature factors " << get_quanser_error_message(result);
            return false;
        }
    };
