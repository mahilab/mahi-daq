    ...
    expire_values(*this, 0), min_values(*this, -10), max_values(*this, 10), m_h(h)
{
    ...
    auto on_write_impl = [this](const ChanNum *chs, const Voltage *vals, std::size_t n) {
        ...
        if (result != 0) {
            LOG(Error) << "Failed to write " << this->get_name() << " analog outputs " << get_quanser_error_message(result);
            return false;
        }
        return true;
    };
    ...
    auto expire_write_impl = [this](const ChanNum* chs, const Voltage* vals, std::size_t n) { 
        ...
        if (result == 0) {
            LOG(Verbose) << "Set " << get_name() << " expire analog expiration states";
            return true;
        }
        else {
            LOG(Error) << "Failed to set " << get_name() << " analog expiration states " << get_quanser_error_message(result);
            return false;
        }        
    };
    ...
    auto min_write_impl = [this](const ChanNum* chs, const Voltage* vals, std::size_t n) { 
        ...
        if (result == 0) {
            LOG(Verbose) << "Set " << get_name() << " analog output ranges"; 
            return true;
        }
        else {
            LOG(Error) << "Failed to set " << get_name() << " analog output ranges " << get_quanser_error_message(result);
            return false;
        }
    };
