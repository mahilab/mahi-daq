...
bool QuanserAO::update() {
    t_error result;
    result = hil_write_analog(daq_.handle_, &channels_internal()[0], static_cast<ChanNum>(channels_internal().size()), &m_values.get()[0]);
    if (result == 0)
        return true;
    else {
...
bool QuanserAO::set_ranges(const std::vector<Voltage>& min_values, const std::vector<Voltage>& max_values) {
    if (!Module::set_ranges(min_values, max_values))
        return false;
    t_error result;
    result = hil_set_analog_output_ranges(daq_.handle_, &channels_internal()[0], static_cast<ChanNum>(channels_internal().size()), &min_values_.get()[0], &max_values_.get()[0]);
    if (result == 0) {
...
bool QuanserAO::set_expire_values(const std::vector<Voltage>& expire_values) {
    if (!Output::set_expire_values(expire_values))
        return false;
    t_error result;
    result = hil_watchdog_set_analog_expiration_state(daq_.handle_, &channels_internal()[0], static_cast<ChanNum>(channels_internal().size()), &expire_values_.get()[0]);
    if (result == 0) {
