...
S826Encoder::S826Encoder(S826& s826, unsigned int board) :
    EncoderModule(s826, {0, 1, 2, 3, 4, 5}), timestamps(*this, util::Time::Zero), m_board(board) {
    // set name
    ...
    auto read_impl = [this](const ChanNum* chs, Counts* valus, std::size_t n) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (result != S826_ERR_OK) {
                LOG(Error) << "Failed to trigger snapshot on " << name() << " channel number "
                           << chs[i] << ". " << sensoray_msg(result);
                success = false;
            }
            result = S826_CounterSnapshotRead(m_board, chs[i], &count, &timestamp, NULL, 0);
            if (result != S826_ERR_OK) {
                LOG(Error) << "Failed to update " << name() << " channel number " << chs[i] << ". "
                           << sensoray_msg(result);
                success = false;
            }
    ...
    auto write_impl = [this](const ChanNum* chs, const Counts* vals, std::size_t n) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (result != S826_ERR_OK) {
                LOG(Error) << "Failed to preload counts in to Preload0 register. "
                           << sensoray_msg(result);
                success = false;
            }
            result = S826_CounterPreload(m_board, chs[i], 1, 0);
            if (result != S826_ERR_OK) {
                LOG(Error) << "Failed to reset counts of " << name() << " channel number " << chs[i]
                           << ". " << sensoray_msg(result);
                success = false;
            }
        }
    ...
    auto quad_write_impl = [this](const ChanNum* chs, const QuadMode* vals, std::size_t n) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (vals[i] == QuadMode::X1)
                ...
                LOG(Error) << "Unsupported quadrature factor (X" << vals[i] << ") requested of "
                           << name();
                success = false;
                continue;
            }
            ...
            if (result != S826_ERR_OK) {
                LOG(Error) << "Failed to set quadrature mode of " << name() << " channel number "
                           << chs[i] << ". " << sensoray_msg(result);
                success = false;
            }
        }
...
bool S826Encoder::on_gain_channels(const ChanNums& chs) {
    bool success = modes.write(chs, std::vector<QuadMode>(chs.size(), QuadMode::X4));
    for (auto& ch : chs) {
        int result = S826_CounterStateWrite(m_board, ch, 1);
        if (result != S826_ERR_OK) {
            LOG(Error) << "Failed to write counter state of " << name() << " channel number " << ch
                       << ". " << sensoray_msg(result);
            success = false;
        }
    }
